...
class ApproveController extends Controller
{
    public function index(){
        $data = DB::select('SELECT * FROM postcost as pc NATURAL JOIN checkapprove as ca NATURAL JOIN post as p NATURAL JOIN users NATURAL JOIN checkapprovestatus where postcostID = 1');
        $c = DB::select('SELECT COUNT(pc.postcostID) as count,pc.postcostID as id FROM postcost as pc NATURAL JOIN checkapprove as ca NATURAL JOIN post as p  GROUP by ca.checkapproveStatusID,pc.postcostID');
        $date = $data[0]->postTime;

        return view('check.index', ['data'=>$data],['postId'=>$c]) ;
    }
    ...
    public function mypost(){
       ...
       # $da = DB::select('SELECT ca.postcostID,pl.placeName,IF(checkapproveStatusID = 1, count(ca.checkapproveStatusID), 0) as one,IF(checkapproveStatusID = 2, count(ca.checkapproveStatusID), 0) as two,IF(checkapproveStatusID = 3, count(ca.checkapproveStatusID), 0) as three FROM postcost as pc NATURAL JOIN post as p  NATURAL JOIN postofgroup as pog NATURAL JOIN groupofdog as god NATURAL JOIN place as pl NATURAL JOIN checkapprove as ca WHERE ca.userID=? group by ca.checkapproveStatusID,pc.postcostID,pl.placeName,ca.postcostID',[1]);
        $data = DB::select('SELECT postcostID,count(ca.checkapproveStatusID) as c ,ca.checkapproveStatusID FROM postcost as pc NATURAL JOIN checkapprove as ca JOIN post as p on pc.postID=p.postID  where p.userID=? group BY pc.postcostID,ca.checkapproveStatusID',[1]);
        $mypost = DB::select('SELECT * FROM postcost as pc NATURAL JOIN post as p  NATURAL JOIN place NATURAL JOIN users NATURAL JOIN typetag NATURAL JOIN subdistrict  NATURAL JOIN postofgroup as pog JOIN groupofdog as grd on grd.groupofdogID= pog.postofgroupID WHERE  p.userID =? and users.id=p.userID and place.placeID=grd.placeID;',[1]);
        $check = array();
        $l=0;
    ...
    public function approve(Request $request){
        ...
        $data = DB::select('SELECT postcostID,count(ca.checkapproveStatusID) as c ,ca.checkapproveStatusID FROM postcost as pc NATURAL JOIN checkapprove as ca JOIN post as p on pc.postID=p.postID  where p.userID=? group BY pc.postcostID,ca.checkapproveStatusID',[1]);
        $mypost = DB::select('SELECT * FROM postcost as pc NATURAL JOIN post as p NATURAL JOIN postofgroup  as pog NATURAL JOIN groupofdog NATURAL JOIN place NATURAL JOIN users NATURAL JOIN typetag NATURAL JOIN subdistrict WHERE  p.userID =? and users.id=p.userID',[1]);

        foreach($mypost as $my){
            foreach($data as $da){
                if($my->postcostID==$da->postcostID){
                    if($da->checkapproveStatusID==2&&$da->c>=3){
                        DB::update('UPDATE post SET poststatusID=1 where postID=?',[2]);
                        break ;
                    }else if ($da->checkapproveStatusID==3&&$da->c>=2){
                        DB::update('UPDATE post SET poststatusID=2 where postID=?',[2]);
                        break ;
                    }
                }
            }
